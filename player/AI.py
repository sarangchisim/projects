...
class AI:
    ...
    def reset(self, gametiles):
        for x in range(8):
            for y in range(8):
                if gametiles[x][y].pieceonTile.tostring() == 'r':
    ...
    def calculateb(self, gametiles):
        ...
        for y in range(8):
            for x in range(8):
                if gametiles[y][x].pieceonTile is None:
                    continue
                ...
                # Piece mobility evaluation (for non-pawn pieces)
                if piece_str.lower() in ['q', 'r', 'b', 'n']:
                    piece = gametiles[y][x].pieceonTile
                    try:
                        if piece_str.islower():
                            moves = piece.legalmoveb(y, x, gametiles) if hasattr(piece, 'legalmoveb') else []
                        else:
                            moves = piece.legalmove(y, x, gametiles) if hasattr(piece, 'legalmove') else []
